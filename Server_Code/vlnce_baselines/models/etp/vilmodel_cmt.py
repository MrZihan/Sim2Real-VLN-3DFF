...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.positional_embedding = nn.Parameter(scale * torch.randn(8 ** 2 + 1, width))

        enc_layer = TransformerEncoderLayer(
            d_model=512, nhead=8, dim_feedforward=2048, dropout=0.1,
                 activation="gelu"
        )
        self.nerf_view_encoder = TransformerEncoder(enc_layer, num_layers=4, norm=BertLayerNorm(512, eps=1e-12), batch_first=True)
        
        args, rgba_mlp, clip_mlp = create_nerf()
        self.nerf_args = args
        self.rgba_mlp = rgba_mlp
        self.clip_mlp = clip_mlp

        self.fcd_position_embedding = nn.Sequential(
            nn.Linear(6, width),
            torch.nn.LayerNorm(width, eps=1e-12)
        )

        self.fcd_aggregation = nn.Sequential(
            nn.Linear(args.featurecloud_search_num*width, width),
            torch.nn.LayerNorm(width, eps=1e-12)
        )
    ...
    def forward_nerf(
        self, scene_memory, position, direction,
    ):
       
        predicted_fts = run_nerf_feature(self.nerf_args, self, scene_memory, position, direction, 8, 8)
