...
class SemanticGrid(object):
    ...
    def update_proj_grid_bayes(self, occup_grid=None, segm_grid=None):
        ...
        for i in range(segm_grid.shape[1]): # sequence
            ...
            #step_segm_grid[:,i,17,:,:] = step_segm_grid[:,i,17,:,:] * torch.square(step_occup_grid[:,i,2,:,:])  # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            #step_segm_grid[:,i,0,:,:] = step_segm_grid[:,i,0,:,:] * torch.square(step_occup_grid[:,i,0,:,:])  # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            #step_segm_grid[:,i,1:17,:,:] = step_segm_grid[:,i,1:17,:,:] * torch.square(step_occup_grid[:,i,1,:,:])   # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            #step_segm_grid[:,i,18:,:,:] = step_segm_grid[:,i,18:,:,:] * torch.square(step_occup_grid[:,i,1,:,:])  # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            #normalization_grid = torch.sum(step_segm_grid[:,i,:,:,:], dim=1, keepdim=True) # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            #step_segm_grid[:,i,:,:,:] = step_segm_grid[:,i,:,:,:] / normalization_grid.repeat(1, segm_grid.shape[2], 1, 1) # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
