    ...
    def _initialize_policy(
        ...
        if load_from_ckpt:
            if config.IL.is_requeue:
                ...
                ckpt_path = config.IL.ckpt_to_load
            ckpt_dict = self.load_checkpoint("data/ckpt.iter12000.pth", map_location="cpu")
            start_iter = ckpt_dict["iteration"]
    ...
    def rollout(self, mode, ml_weight=None, sample_ratio=None):
        ...
        for stepk in range(self.max_len):
            ...
            if self.config.GPU_NUMBERS > 1:
                ...
                step_segm_occup_grid_maps = torch.cat((predicted_segm_grid_maps.unsqueeze(1),predicted_occup_grid_maps.unsqueeze(1)),dim=-3)
                waypoint_grid_maps = self.policy.net.waypoint_predictor(step_segm_occup_grid_maps).view(batch_size,1,map_config['grid_dim'][0],map_config['grid_dim'][1]).squeeze(1)
            ...
            waypoint_grid_maps = waypoint_grid_maps
            ...
            for b in range(self.batch_size):                
                ...
                fig = plt.gcf()
                plt.margins(0,0)
                fig.savefig("example/"+str(b)+'_'+str(stepk)+'reponse_map.png', dpi=500, bbox_inches='tight')
                plt.show()

                ################################## 

                save_img_dir = "example/" + 'occup_' + str(b) +'_'
                viz_utils.write_tensor_imgSegm(predicted_occup_grid_maps, save_img_dir, name="img_segm", t=stepk, labels=3)


                save_img_dir = "example/" + 'img_' + str(b) +'_'
                im_path = save_img_dir + "_" + str(stepk) + ".png"

                Image.fromarray(viz_img).save(im_path)
                                       

                save_img_dir = "example/" + 'predicted_cwp_' + str(b) +'_'
