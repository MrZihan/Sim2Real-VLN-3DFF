...
class GraphMap(object):
    ...
    def update_graph_no_overlap(self, prev_vp, step_id,
                           cur_vp, cur_pos, cur_embeds,
                           cand_vp, cand_pos, cand_embeds, cand_angles,
                           cand_real_pos):
        # 1. connect prev_vp
        self.graph_nx.add_node(cur_vp)
        if prev_vp is not None:
        ...
        for i, (cvp, cpos, cembeds, cangles) in enumerate(zip(cand_vp, cand_pos, cand_embeds, cand_angles)):

            if i != 0 and i != len(cand_angles)-1 and 1/2 * math.pi < cangles and cangles < 3/2 * math.pi:
                continue
